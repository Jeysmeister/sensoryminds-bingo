...
const BingoItem = ({ index, item, marked, setMarked, questions }: { index: number; item: string; marked: number[]; setMarked: (value: number[]) => void; questions: string[] }) => {
  ...
  return (
    <motion.div
      ...
      key={index}
      className={`relative flex items-center justify-center p-2 h-[70px] w-[70px] xs:h-[80px] xs:w-[80px] sm:h-[100px] sm:w-[100px] border border-gray-200 rounded ${
        active ? "bg-black text-white" : "bg-white text-black"
      } cursor-pointer`}
      onClick={() => handleClick(index)}
    >
      <p className="text-center text-2xs sm:text-xs font-extrabold capitalize">{item}</p>
    </motion.div>
  );
};
