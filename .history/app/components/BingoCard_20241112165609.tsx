    ...
    <div className="border border-gray-300 p-4 pt-0 rounded-xl">
      ...
      <div className="max-w-[800px] max-h-[800px] grid grid-cols-5 gap-2">
        {grid.map((item, index) => (
          <div
            key={index}
            className={`relative flex items-center justify-center p-2 h-[130px] w-[130px] bg-white border border-gray-200 rounded ${marked.includes(index) ? "text-white" : "bg-white text-black"}`}
