...
const BingoItem = ({ index, item, marked, setMarked }: { index: number; item: string; marked: number[]; setMarked: (value: number[]) => void }) => {
  ...
  return (
    <motion.div
      ...
      key={index}
      className={`relative flex items-center justify-center p-2 h-[130px] w-[130px] border border-gray-200 rounded ${active ? "bg-black text-white" : "bg-white text-black"} cursor-pointer`}
