...
const BingoItem = ({ index, item, marked, setMarked, questions }: { index: number; item: string; marked: number[]; setMarked: (value: number[]) => void; questions: string[] }) => {
  ...
  return (
    <motion.div
      ...
      key={index}
      className={`relative flex items-center justify-center p-2 h-[75px] w-[75px] border border-gray-200 rounded ${active ? "bg-black text-white" : "bg-white text-black"} cursor-pointer`}
