...
const BingoItem = ({ index, item, marked, setMarked }: { index: number; item: string; marked: number[]; setMarked: (value: number[]) => void }) => {
  ...
  return (
    <motion.div
      initial={{ backgroundColor: "#ffffff" }} // Initial color (white)
      animate={{
        backgroundColor: active ? "#000000" : "#ffffff", // Change color based on 'active' state
      }}
      transition={{ duration: 0.5 }} // Duration of color change
      key={index}
