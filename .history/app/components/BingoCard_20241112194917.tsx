...
const BingoCard = ({ questions }: { questions: string[] }) => {
  ...
  return (
    <>
      <motion.div variants={container} initial="hidden" animate="visible" className="border border-gray-300 p-4 m-4 pt-0 rounded-xl">
        <div className="flex w-full justify-center items-center py-4 text-xl font-extrabold">NEVER HAVE I BINGO</div>
        <div className="max-w-[800px] max-h-[800px] w-screen grid grid-cols-5 gap-1">
          {grid.map((item, index) => (
